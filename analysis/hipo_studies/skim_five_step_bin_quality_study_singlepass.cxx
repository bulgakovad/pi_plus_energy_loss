...
//
//   Route each accepted Step-1 selected track into exactly one theta/p bin.
...
//   - assign selected track into exactly one theta/p grid bin
//
// Step 2.1:
...
//
// Default grid:
//   theta: [30, 42) deg, step 1.0 deg
//   p:     [0.4, 4.8) GeV, step 0.2 GeV
//
// Example:
//
// clas12root -l -b -q 'skim_five_step_bin_quality_study_singlepass.cxx+("good_hipo.dat","bin_quality_studies","FD",30,42,1.0,0.4,4.8,0.2,-0.5,0.5,200,-1)'
...
static bool get_mclund_pip_momentum(hipo::bank& mc_lund,
                                    double& p_lund,
                                    int& n_lund_pip)
{
...
{
    const int rc = gSystem->mkdir(folder.c_str(), true);

    if (rc != 0 && gSystem->AccessPathName(folder.c_str())) {
                        ...
                        int n_bins)
{
    ...
    const int bin =
        static_cast<int>(std::floor((value - grid_min) / step));
                          ...
                          long long n_entries)
{
    ...
    const double ymax = std::max(1.0, h->GetMaximum() * 1.05);

    TLine* zero = new TLine(0.0, 0.0, 0.0, ymax);
    zero->SetLineStyle(2);
    zero->SetLineWidth(2);
    zero->Draw("same");
...
static void update_step21_multiplicity(BinData& bin,
                                       int n_rec_pip_this_event,
                                       int n_positive_this_event)
{
    bin.n_21_total_rec_pip_rows += n_rec_pip_this_event;
    bin.n_21_total_positive_rows += n_positive_this_event;
...
static void write_bin_stats(const BinData& bin,
                            ...
                            long long global_one_lund_pip)
{
    const std::string stats_path =
        join_path(bin.folder, "bin_stats.txt");
    ...
    out << "Theta bin:       ["
        ...
        << format_number(bin.theta_max) << ") deg\n";
    out << "Momentum bin:    ["
        << format_number(bin.p_min) << ", "
        << format_number(bin.p_max) << ") GeV\n\n";
    ...
    out << std::left << std::setw(72)
        ...
        << "   "
        << std::fixed << std::setprecision(3)
        << pct(bin.n_21_zero_rec_pip, bin.n_21)
        << " %\n";
    ...
    out << std::left << std::setw(72)
        ...
        << "   "
        << std::fixed << std::setprecision(3)
        << pct(bin.n_21_one_rec_pip, bin.n_21)
        << " %\n";
    ...
    out << std::left << std::setw(72)
        ...
        << "   "
        << std::fixed << std::setprecision(3)
        << pct(bin.n_21_one_rec_pip_only_positive, bin.n_21)
        << " %\n";
    ...
    out << std::left << std::setw(72)
        ...
        << "   "
        << std::fixed << std::setprecision(3)
        << pct(bin.n_21_one_rec_pip_extra_positive, bin.n_21)
        << " %\n";
    ...
    out << std::left << std::setw(72)
        ...
        << "   "
        << std::fixed << std::setprecision(3)
        << pct(bin.n_21_gt1_rec_pip, bin.n_21)
        << " %\n";
    ...
    out << "No other positive tracks: "
        ...
        << " = "
        << std::fixed << std::setprecision(3)
        << pct(bin.n_21_one_rec_pip_only_positive,
               bin.n_21_one_rec_pip)
        << " %\n";
    ...
    out << "Extra positive tracks:    "
        ...
        << " = "
        << std::fixed << std::setprecision(3)
        << pct(bin.n_21_one_rec_pip_extra_positive,
               bin.n_21_one_rec_pip)
        << " %\n";
    ...
    out << "Step 1 entries:   "
        ...
        << "   mean=" << bin.h_step1->GetMean()
        << "   stddev=" << bin.h_step1->GetStdDev()
        << "\n";

    out << "Step 2.1 entries: "
        << bin.h_21->GetEntries()
        << "   mean=" << bin.h_21->GetMean()
        << "   stddev=" << bin.h_21->GetStdDev()
        << "\n";

    out << "Step 2.2 entries: "
        << bin.h_22->GetEntries()
        << "   mean=" << bin.h_22->GetMean()
        << "   stddev=" << bin.h_22->GetStdDev()
        << "\n";

    out << "Step 3.1 entries: "
        << bin.h_31->GetEntries()
        << "   mean=" << bin.h_31->GetMean()
        << "   stddev=" << bin.h_31->GetStdDev()
        << "\n";

    out << "Step 3.2 entries: "
        << bin.h_32->GetEntries()
        << "   mean=" << bin.h_32->GetMean()
        << "   stddev=" << bin.h_32->GetStdDev()
        << "\n";
...
void skim_five_step_bin_quality_study_singlepass(
    ...
    const char* output_root_folder = "bin_quality_studies",
...
{
    const std::string det = detector;
    const std::string output_root = output_root_folder;
    ...
    const int n_theta_bins =
        static_cast<int>(std::llround(
            (theta_grid_max - theta_grid_min) / theta_step
        ));

    const int n_p_bins =
        static_cast<int>(std::llround(
            (p_grid_max - p_grid_min) / p_step
        ));

    const double theta_span_rebuilt =
        theta_grid_min + n_theta_bins * theta_step;

    const double p_span_rebuilt =
        p_grid_min + n_p_bins * p_step;

    if (std::fabs(theta_span_rebuilt - theta_grid_max) > 1.0e-9 ||
        std::fabs(p_span_rebuilt - p_grid_max) > 1.0e-9) {
        std::cerr << "ERROR: grid range is not divisible by grid step.\n";
        return;
    }

    std::cout << "Macro started: FIVE-STEP MULTI-BIN QUALITY STUDY\n";
    std::cout << "Input .dat file:       " << input_dat_file << "\n";
    std::cout << "Output root folder:    " << output_root << "\n";
    std::cout << "Detector:              " << det << "\n";
    std::cout << "Theta grid:            ["
              << theta_grid_min << ", "
              << theta_grid_max << ") deg, step "
              << theta_step << "\n";
    std::cout << "Momentum grid:         ["
              << p_grid_min << ", "
              << p_grid_max << ") GeV, step "
              << p_step << "\n";
    std::cout << "Theta bins:            " << n_theta_bins << "\n";
    std::cout << "Momentum bins:         " << n_p_bins << "\n";
    ...
    auto hipo_files_raw =
        get_hipo_files_from_dat(input_dat_file);

    if (hipo_files_raw.empty()) {
        std::cerr << "ERROR: no .hipo files found in "
                  << input_dat_file << "\n";
        return;
    }
    ...
    bins.reserve(n_theta_bins * n_p_bins);

    for (int i_theta = 0; i_theta < n_theta_bins; i_theta++) {
        const double theta_min =
            theta_grid_min + i_theta * theta_step;

        const double theta_max =
            theta_min + theta_step;

        for (int i_p = 0; i_p < n_p_bins; i_p++) {
            const double p_min =
                p_grid_min + i_p * p_step;
            ...
            bin.theta_min = theta_min;
            bin.theta_max = theta_max;
            bin.p_min = p_min;
            bin.p_max = p_max;
            ...
            bin.folder =
                join_path(output_root, subfolder_name);
            ...
            bin.h_step1 = new TH1D(
                ("h_step1_" + unique).c_str(),
                "",
                n_dp_bins,
                dp_min,
                dp_max
    ...
    for (size_t ifile = 0;
         ifile < hipo_files.size() && !reached_max_events;
         ifile++) {
        ...
        if (!get_schema_safe(factory,
                             "REC::Particle",
                             rec_schema)) {
            std::cerr << "WARNING: REC::Particle missing, skipping:\n"
                      << "  " << input_hipo << "\n";
        ...
        if (!get_schema_safe(factory,
                             "MC::Lund",
                             mclund_schema)) {
            std::cerr << "WARNING: MC::Lund missing, skipping:\n"
                      << "  " << input_hipo << "\n";
        ...
        while (reader.next()) {
            ...
            for (int i = 0; i < rec_part.getRows(); i++) {
                const int pid =
                    rec_part.getInt("pid", i);

                const int charge =
                    rec_part.getInt("charge", i);

                const int status =
                    rec_part.getInt("status", i);

                const double px =
                    rec_part.getFloat("px", i);

                const double py =
                    rec_part.getFloat("py", i);

                const double pz =
                    rec_part.getFloat("pz", i);
            ...
            const int i_theta =
                get_grid_bin(selected_step1.theta_deg,
                             theta_grid_min,
                             theta_grid_max,
                             theta_step,
                             n_theta_bins);
            ...
            const int flat_bin =
                i_theta * n_p_bins + i_p;

            BinData& bin =
                bins.at(flat_bin);

            const double dp =
                selected_step1.p - p_lund;
    ...
    for (auto& bin : bins) {
        ...
        save_hist_png(
            bin.h_step1,
            join_path(bin.folder, "step1_dp.png"),
            "Step 1: selected detector-positive track;#Delta p = p_{rec}(selected positive) - p_{Lund}(#pi^{+}) [GeV];Counts",
            bin.n_step1
        );

        save_hist_png(
            bin.h_21,
            join_path(bin.folder, "step2p1_dp.png"),
            "Step 2.1: selected Step-1 track is #pi^{+};#Delta p = p_{rec}(selected #pi^{+}) - p_{Lund}(#pi^{+}) [GeV];Counts",
            bin.n_21
        );

        save_hist_png(
            bin.h_22,
            join_path(bin.folder, "step2p2_dp.png"),
            "Step 2.2: exactly one positive track total;#Delta p = p_{rec}(selected positive) - p_{Lund}(#pi^{+}) [GeV];Counts",
            bin.n_22
        );

        save_hist_png(
            bin.h_31,
            join_path(bin.folder, "step3p1_dp.png"),
            "Step 3.1: Step-2.1 pion is only positive track;#Delta p = p_{rec}(selected #pi^{+}) - p_{Lund}(#pi^{+}) [GeV];Counts",
            bin.n_31
        );

        save_hist_png(
            bin.h_32,
            join_path(bin.folder, "step3p2_dp.png"),
            "Step 3.2: Step-2.2 only positive track is #pi^{+};#Delta p = p_{rec}(selected #pi^{+}) - p_{Lund}(#pi^{+}) [GeV];Counts",
            bin.n_32
        );
