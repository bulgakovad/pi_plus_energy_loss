...
# --- desired pi+ phase-space triangle (p vs theta) ---
PPI_MIN = 0.1
PPI_MAX = 9.0
THPI_MIN_DEG = 5.0
THPI_MAX_DEG = 140.0
...
def sample_uniform_in_right_triangle(p_min, p_max, th_min_deg, th_max_deg):
    """
    Uniform in area inside right triangle with vertices:
      (p_min, th_min), (p_min, th_max), (p_max, th_min)
    Uses the reflection trick (no rejection loop).
    """
    u = ROOT.gRandom.Uniform(0.0, 1.0)
    v = ROOT.gRandom.Uniform(0.0, 1.0)

    # reflect points above the diagonal into the triangle
    if u + v > 1.0:
        u = 1.0 - u
        v = 1.0 - v

    p = p_min + u * (p_max - p_min)
    th_deg = th_min_deg + v * (th_max_deg - th_min_deg)
    return p, th_deg
...
def generate_lund_file(file_index, output_folder, num_events_per_file, Ebeam):
    ...
    for _ in range(num_events_per_file):
        ...
        # ---- pi+ sampled uniformly in the right-triangle region in (p, theta) ----
        p_pi, th_pi_deg = sample_uniform_in_right_triangle(
            PPI_MIN, PPI_MAX, THPI_MIN_DEG, THPI_MAX_DEG
        )

        th_pi = th_pi_deg * DEG2RAD
...
if __name__ == "__main__":
    outdir = "/lustre24/expphy/volatile/clas12/bulgakov/lund_files/piplus_electron_lund_multithread_triang_region"
    Ebeam = 10.6
